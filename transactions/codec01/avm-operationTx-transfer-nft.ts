...
const main = async (): Promise<any> => {
  ...
  utxos.forEach((utxo: UTXO) => {
    ...
    if(utxo.getOutput().getTypeID() != 10) {
      ...
      const nftTransferOutputUTXOIDs: string[] = getUTXOIDs(utxoSet, bintools.cb58Encode(txid), AVMConstants.NFTXFEROPID, bintools.cb58Encode(assetID))
      const nftTransferOutput = utxo.getOutput() as NFTTransferOutput
      const spenders: Buffer[] = nftTransferOutput.getSpenders(xAddresses)
      const outbound:NFTTransferOutput = new NFTTransferOutput(
        nftTransferOutput.getGroupID(), payload, xAddresses, locktime, threshold, 
      )
      const nftTransferOperation: NFTTransferOperation = new NFTTransferOperation(outbound)

      spenders.forEach((spender: Buffer) => {
        const idx: number = nftTransferOutput.getAddressIdx(spender)
        nftTransferOperation.addSignatureIdx(idx, spender)
      })
    
      const transferableOperation: TransferableOperation = new TransferableOperation(utxo.getAssetID(), nftTransferOutputUTXOIDs, nftTransferOperation)
